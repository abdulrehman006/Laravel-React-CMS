...
export default function Index({ notifications, sort }) {
    ...
    const markAll = () => {
        if (isMarkAll) {
            ...
            const items = notifications.data.map((notification) => notification.id);
            setMarkItems(items);
            setIsMarkAll(true);
        }
    };
    ...
    const handleMark = (notificationId) => {
        const existsMark = markItems.some((item) => item === notificationId);
        if (existsMark) {
            const removeItem = markItems.filter((item) => item !== notificationId);
            setMarkItems(removeItem);
        } else {
            const addedItem = [...markItems, notificationId];
    ...
    const handleBulkAction = () => {
        ...
        if (selectedOption === "Delete") {
            confirmMessage = "Do You want to delete selected notifications?";
            action = "admin.notifications.bulk.delete";
        }
        setIsMarkAll([]);
            ...
            <AdminLayouts>
                ...
                <div className="container-fluid">
                    <div className="yoo-uikits-heading">
                        <h2 className="yoo-uikits-title">All notifications</h2>
                    </div>
                                                    ...
                                                    (notification, index) => (
                                                        ...
                                                        >
                                                            ...
                                                            <td className="yoo-table-medias yoo-style1">
                                                                <a
                                                                href={route("pricing.plan", notification)}
                                                                target="_blank"
                                                                >
                                                                    {notification.title}
                                                                </a>
                                                            ...
                                                            <td>
                                                                ...
                                                                >
                                                                    <Link
                                                                            href={route(
                                                                                "admin.notifications.edit", notification
                                                                            )}
